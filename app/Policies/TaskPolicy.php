...
class TaskPolicy
{
     ...
     *
     * `currentAccessToken() !== null` : la restriction ne s'applique que
     * pour une authentification par vrai jeton API. Sans cette condition,
     * `tokenCan()` vaut toujours `false` pour une session web classique
     * (guard `web`, `currentAccessToken()` toujours `null`) — bug réel qui
     * bloquait toute l'interface web, trouvé en écrivant les tests de
     * policies du Module 10 (voir ProjectPolicy et ChecksTokenAbility pour
     * le détail).
     */
